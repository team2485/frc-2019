...
public class CargoGroundIntake extends Command {
  //constants
    long intakeTime; 
    ...
    @Override 
    protected boolean isFinished() { //Fix
      currentCurrent = RobotMap.cargoIntake.current();
        ...
        if(isSpiking) {
          if (RobotMap.cargoIntake.current() < currentCurrent / minSpikeDecrease) {
            isSpiking = false;
          }

          if (System.currentTimeMillis() - start >= minTimeSpike) {
            spike = true;
            isSpiking = false;
          }
        }
